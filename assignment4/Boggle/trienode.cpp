...
void TrieNode::addWord(const std::string &word) {
    if (word.empty() || word.front() == '\r') {
        ...
        if (children.find(word.front()) != children.end()) {
            ...
            children[word.front()] = std::make_unique<TrieNode>(word.front(), false);
            children[word.front()]->addWord(word.substr(1));
        }
    }
}
...
std::unique_ptr<TrieNode> &TrieNode::getPrefixedNode(const std::string &prefix) {
    if (prefix.size() - 1 == 0) {
